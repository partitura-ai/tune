...
//
// Modified for CLI usage: replaced Bundle-based model loading with directory-based loading,
// replaced auto-generated fastvithd class with manual MLModel loading.
...
private enum Vision {
    ...
    /// Loads the fastvithd CoreML model from a .mlpackage directory
    fileprivate class VisionModelCoreML {
        ...
        public func encode(_ image: MLXArray) -> MLXArray {
            ...
            return data.withUnsafeMutableBytes { (ptr: UnsafeMutableRawBufferPointer) in
                ...
                let outputArray = output.featureValue(for: "image_features")!.multiArrayValue!
                precondition(outputArray.shape == [1, 256, 3072])
                precondition(outputArray.dataType == .float32)
                return outputArray.withUnsafeBytes { ptr in
    ...
    fileprivate class VisionModel: Module {
        ...
        let coremlModel: VisionModelCoreML

        public init(modelPath: URL) {
            self.coremlModel = VisionModelCoreML(modelPath: modelPath)
        }

        public func callAsFunction(_ hiddenStates: MLXArray, gridThw: [THW]) -> MLXArray {
            coremlModel.encode(hiddenStates)
        }
    }
}
...
public class FastVLMProcessor: UserInputProcessor {
    ...
    public func prepare(prompt: UserInput.Prompt, imageTHW: THW?) -> String {
        ...
        case .messages(let msgs):
            promptText = (msgs.last?["content"] as? String) ?? ""
        case .chat(let chatMsgs):
            promptText = chatMsgs.last?.content ?? ""
        @unknown default:
            promptText = "\(prompt)"
        }
...
/// FastVLM model — CLI version that loads from a directory path
public class FastVLMModel: Module, VLMModel, KVCacheDimensionProvider {
    ...
    public var defaultLoraKeys: [String] {
        ["q_proj", "v_proj"]
    }

    public init(_ config: FastVLMConfiguration, modelDirectory: URL) {
        self.config = config
        let mlpackagePath = modelDirectory.appendingPathComponent("fastvithd.mlpackage")
        self._visionModel.wrappedValue = Vision.VisionModel(modelPath: mlpackagePath)
    ...
    public func sanitize(weights: [String: MLXArray]) -> [String: MLXArray] {
        _ = try? visionModel.coremlModel.load()
        return weights
    }
}
